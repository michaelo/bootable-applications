...
void plasma(float *staticData, Bitmap *backBuffer, float time, int interlacing)
{
    ...
    for (int y = 0; y < h; y += 1)
    {
        for (int x = 0; x < w; x += 1)
        {
            ...
            float value =
                staticData[pos] + sin(dist(x, y, p3x, yPos) / scaleFactor) // Distance from point p3, which is moving vertically
...
void bltBitmapFit(Bitmap *target, Bitmap *source, EFI_UINTN x_start, EFI_UINTN y_start, EFI_UINTN x_end, EFI_UINTN y_end)
{
...
EFI_UINTN EfiMain(EFI_HANDLE handle, EFI_SYSTEM_TABLE *system_table)
{
    ...
    for (;;)
    {
        ...
        // TOOD: add eventlistener for key (up/down) to increase/decrease resolution of plasmabuffer? Must also reinitialize static data.

        // Render plasma and text to low-res bitmap
        plasma(staticData, plasmaBuffer, t, interlacing);
        scrollingText(plasmaBuffer, cache, &memory, t, text, 71);

        // Scale lowres bitmap to screen-sized backbuffer
        bltBitmapFit(backBuffer, plasmaBuffer, 0, 0, backBuffer->width, backBuffer->height);
